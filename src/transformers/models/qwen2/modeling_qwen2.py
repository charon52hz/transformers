...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights_minus = -1 * attn_weights #########################chz
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask
        attn_weights_minus = attn_weights_minus + causal_mask #########################chz

    attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query.dtype)
    attn_weights_minus = nn.functional.softmax(attn_weights_minus, dim=-1, dtype=torch.float32).to(query.dtype)###chz
    attn_weights = attn_weights - attn_weights_minus    ################chz
